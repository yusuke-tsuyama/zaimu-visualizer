...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { extractedText, fileName, fiscalYear, unit, statementType, accountItems, base64Pdf, sessionId } = body

    const limit = await enforceRateLimit(ip, sessionId ?? null)
    if (!limit.ok) {
      const message = limit.reason === 'session'
        ? '本日の利用上限（15期分）に達しました。明日また試してください。'
